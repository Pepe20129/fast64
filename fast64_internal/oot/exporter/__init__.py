...
from ..oot_f3d_writer import writeTextureArraysNew, writeTextureArraysNewXML, writeTextureArraysExisting1D
...
class SceneExport:
    ...
    def export_xml(originalSceneObj: Object, transform: Matrix, exportInfo: ExportInfo, logging_func):
        ...
        logging_func({"INFO"}, "ootExportSceneToXML 4.5")

        exportPath = exportInfo.exportPath

        logging_func({"INFO"}, "ootExportSceneToXML 4.6")

        for section in resourceBasePath.split("/"):
            logging_func({"INFO"}, "ootExportSceneToXML 4.7")
            exportPath = os.path.join(exportPath, section)
            logging_func({"INFO"}, "ootExportSceneToXML 4.8")

        logging_func({"INFO"}, "ootExportSceneToXML 4.9")

        path = ootGetPath(exportPath, True, exportSubdir, scene.name, True, True)

        logging_func({"INFO"}, "ootExportSceneToXML 4.10")

        textureExportSettings = TextureExportSettings(False, exportInfo.saveTexturesAsPNG, sceneInclude, path)

        logging_func({"INFO"}, "ootExportSceneToXML 4.11")

        sceneFile = scene.getNewSceneFileXML(path, exportInfo.isSingleFile, textureExportSettings, resourceBasePath, logging_func)

        logging_func({"INFO"}, "ootExportSceneToXML 5.1")

        textureArrayData = writeTextureArraysNewXML(scene.model, None)
        sceneFile.sceneTextures += textureArrayData

        logging_func({"INFO"}, "ootExportSceneToXML 5.2")
        ...
        #    + (levelXML.sceneTextures if levelXML.sceneTextures is not None else "None")
        #)
        logging_func(
            {"INFO"},
        ...
        for i in range(len(scene.rooms.entries)):
            ...
            #for meshEntry in scene.rooms.entries[i].mesh.meshEntries:
                ...
                #    writeXMLData(
                #        meshEntry.DLGroup.opaque.to_xml(None, "{resource_base_path}").replace(
                #            "{resource_base_path}", resourceBasePath[:-1]
                #        ),
                ...
                #    writeXMLData(
                #        meshEntry.DLGroup.transparent.to_xml(None, "{resource_base_path}").replace(
                #            "{resource_base_path}", resourceBasePath[:-1]
                #        ),
